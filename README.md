...
1. Приложение реализовано в рамках требований тестового задания. История переписки не сохраняется после перезагрузки страницы. 
2. Для запросов используется универсальный хост GREEN-API https://api.green-api.com, поэтому для запуска достаточно указать idInstance и apiTokenInstance, как предусмотрено тестовым заданием.